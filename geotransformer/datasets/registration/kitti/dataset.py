...
class OdometryKittiPairDataset(torch.utils.data.Dataset):
    ...
    def _load_point_cloud(self, file_name,pcd,label_path):
        points = np.load(file_name)
        _,seq,pc= pcd.split('/')
        label_path = osp.join(label_path,seq,pc)
    ...
    def __getitem__(self, index):
        ...
        ref_points, ref_labels= self._load_point_cloud(osp.join(self.dataset_root, metadata['pcd0']),metadata['pcd0'],self.label_path)
        src_points,src_labels = self._load_point_cloud(osp.join(self.dataset_root, metadata['pcd1']),metadata['pcd1'],self.label_path)
        ...
        data_dict['ref_labels'] = ref_labels.astype(np.float32)
        data_dict['src_labels'] = src_labels.astype(np.float32)
