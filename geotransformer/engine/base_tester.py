...
def inject_default_parser(parser=None):
    ...
    # parser.add_argument('--snapshot', default='/data/xzy/gxy/geo/output/2-correct_image_self_cross_BOW/snapshots/epoch-29.pth.tar', help='load from snapshot')
...
class BaseTester(abc.ABC):
    def __init__(self, cfg, parser=None, cudnn_deterministic=True):
        ...
        # logger
        log_file = osp.join(cfg.log_dir, str(self.args.test_epoch)+'-test-{}.log'.format(time.strftime('%Y%m%d-%H%M%S')))
