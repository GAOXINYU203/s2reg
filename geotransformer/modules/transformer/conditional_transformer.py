...
from geotransformer.modules.transformer.semantic_transformer import SemanticTransformerLayer


def _check_block_type(block):
    if block not in ['self', 'cross', 'label_self', 'label_cross']:
    ...
    def __init__(
        ...
        for block in self.blocks:
            ...
            if block == 'self':
                ...
                layers.append(TransformerLayer(d_model, num_heads, dropout=dropout, activation_fn=activation_fn))
            elif block == 'label_self':
                layers.append(SemanticTransformerLayer(d_model, num_heads, dropout=dropout, activation_fn=activation_fn))
            elif block == 'label_cross':
                layers.append(PETransformerLayer(d_model, num_heads, dropout=dropout, activation_fn=activation_fn))
        self.layers = nn.ModuleList(layers)
        self.return_attention_scores = return_attention_scores
        self.parallel = parallel

    def forward(self, feats0, feats1, embeddings0, embeddings1,label0,label1, masks0=None, masks1=None):
        attention_scores = []
        for i, block in enumerate(self.blocks):
            if block == 'self':
                ...
                feats0, scores0 = self.layers[i](feats0, feats0, embeddings0, label0, memory_masks=masks0)
                feats1, scores1 = self.layers[i](feats1, feats1, embeddings1, label1, memory_masks=masks1)
            elif block == 'label_cross':
                feats0, scores0 = self.layers[i](feats0, feats1, label0, label1, memory_masks=masks1)
                feats1, scores1 = self.layers[i](feats1, feats0, label1, label0, memory_masks=masks0)
            if self.return_attention_scores:
                attention_scores.append([scores0, scores1])
        if self.return_attention_scores:
            return feats0, feats1, attention_scores
