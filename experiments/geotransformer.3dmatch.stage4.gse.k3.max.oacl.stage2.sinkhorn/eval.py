...
import torch
import numpy as np
from mac import registration_with_maximal_cliques
...
def make_parser():
    ...
    parser.add_argument('--method', choices=['lgr', 'ransac', 'svd','mac'], required=True, help='registration method')
    ...
    for scene_root in scene_roots:
        ...
        for file_name in file_names:
            ...
            if args.method == 'lgr':
                ...
                sel_indices = np.argsort(-corr_scores)
                src_tmp_points = src_corr_points[sel_indices]
                ref_tmp_points = ref_corr_points[sel_indices]
                src_corr_points = np.array([src_tmp_points[0]])
                ref_corr_points = np.array([ref_tmp_points[0]])
                for i in range(1, src_tmp_points.shape[0]):
                    if src_tmp_points[i] not in src_corr_points and ref_tmp_points[i] not in ref_corr_points:
                        src_corr_points = np.append(src_corr_points, np.array([src_tmp_points[i]]), axis=0)
                        ref_corr_points = np.append(ref_corr_points, np.array([ref_tmp_points[i]]), axis=0)
                with torch.no_grad():
                    ref_corr_points = torch.from_numpy(ref_corr_points).cuda()
                    src_corr_points = torch.from_numpy(src_corr_points).cuda()
                    estimated_transform = registration_with_maximal_cliques(src_corr_points, ref_corr_points)
...
def main():
    ...
    cfg = make_cfg()
    log_file = osp.join(cfg.log_dir, 'eval' + str(args.test_epoch) + '-'+ str(args.benchmark)+ '-' + args.method + '-{}.log'.format(time.strftime('%Y%m%d-%H%M%S')))
