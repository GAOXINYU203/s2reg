...
from geotransformer.modules.transformer.superpoint_class import new_kitti_superpoint_label
...
class GeoTransformer(nn.Module):
    def __init__(self, cfg):
        ...
        self.num_points_in_raw = cfg.model.num_points_in_raw
    ...
    def forward(self, data_dict):
        ...
        ref_class = data_dict['ref_labels'].detach()
        src_class =data_dict['src_labels'].detach() 
        ...
        _, ref_node_masks_cla, ref_node_knn_indices_cla, ref_node_knn_masks_cla = point_to_node_partition(
            ref_points, ref_points_c, self.num_points_in_raw
        )
        _, src_node_masks_cla, src_node_knn_indices_cla, src_node_knn_masks_cla = point_to_node_partition(
            src_points, src_points_c, self.num_points_in_raw
        )
        ref_points_cla = torch.cat((ref_points,ref_class),dim=1)
        src_points_cla = torch.cat((src_points,src_class),dim=1)
        ref_padded_points = torch.cat([ref_points_cla, torch.zeros_like(ref_points_cla[:1])], dim=0)
        src_padded_points = torch.cat([src_points_cla, torch.zeros_like(src_points_cla[:1])], dim=0)
        ref_node_knn_points_cla = index_select(ref_padded_points, ref_node_knn_indices_cla, dim=0)
        src_node_knn_points_cla = index_select(src_padded_points, src_node_knn_indices_cla, dim=0)
        ref_node_knn_points_cla = ref_node_knn_points_cla[:,:,3]
        src_node_knn_points_cla = src_node_knn_points_cla[:,:,3]
        ...
        ref_points_c_BOW = new_kitti_superpoint_label(ref_node_knn_points_cla).cuda()
        src_points_c_BOW = new_kitti_superpoint_label(src_node_knn_points_cla).cuda()
        # 2. KPFCNN Encoder
        ...
        ref_feats_c, src_feats_c = self.transformer(
            ...
            ref_points_c_BOW.unsqueeze(0),
            src_points_c_BOW.unsqueeze(0),
        )
